...
echo
echo Response
curl -X POST "http://api.authy.com/onetouch/xml/users/$AUTHY_ID/approval_requests?api_key=$AUTHY_API_KEY" -d message="$OT_MESSAGE" -d details="$OT_DETAILS" -d seconds_to_expire="$OT_TTL"
echo
