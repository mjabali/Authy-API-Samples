...
echo
echo Request
echo curl "http://api.authy.com/protected/onetouch/\$AUTHY_API_FORMAT/users/\$AUTHY_ID/approval_requests?api_key=\$AUTHY_API_KEY" 
...
echo
echo Response
curl "http://api.authy.com/onetouch/$AUTHY_API_FORMAT/users/$AUTHY_ID/approval_requests?api_key=$AUTHY_API_KEY" -d message="$OT_MESSAGE" -d details="$OT_DETAILS" -d seconds_to_expire="$OT_TTL"
echo
